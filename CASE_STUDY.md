...
[jaxPOSEIDON](https://github.com/handley-lab/jaxPOSEIDON) is the result: a Phase 0–10 port covering POSEIDON's v0 transmission envelope, extended in v0.5 to POSEIDON's full forward-model surface (transmission / emission / reflection with single-stream + Toon two-stream solvers, MacMad17 + Mie clouds, isochem + FastChem equilibrium chemistry, MS09 / Madhu / Pelletier / `PT_penalty` P-T profiles, 1D / 2D / 3D atmospheres, line-by-line opacity mode, photometric + spectroscopic instruments, surfaces, stellar contamination, high-resolution data-prep / CCF, and per-species / per-layer contribution kernels), validated against POSEIDON across **1,744 tests** including a **396-case forward-model parametric sweep** at `atol=1e-15, rtol=1e-13` — comfortably inside the plan's `≤1 ppm` binned-spectrum target. JAX-traceable forward model and live BlackJAX NSS execution are explicitly v1 work items.

The v0.5 work was staged as Phases 0.5.1–0.5.17 over a follow-up session: setup-API mirror, free-gravity / mass parsing, surfaces, H- bound-free / free-free, photometric instruments, multi-knot P-T profiles, 2D/3D atmospheres + Δ-mixing-ratio gating, FastChem equilibrium-chemistry grid, CLR priors, stellar contamination, Mie aerosols, emission + Toon two-stream + reflection, surface spectral coupling in emission, line-by-line opacity loader + `extinction_LBL` kernel, high-resolution data-prep + Doppler + CCF, save_spectrum / disable_continuum / contribution kernels, and the docs refresh in this PR. POSEIDON's open-source `Iceberg` cloud branch is **absent at the cloned commit** — jaxPOSEIDON rejects `opaque_Iceberg=True` with `NotImplementedError` because there is no POSEIDON output to match. eddysed runtime wiring and the high-res retrieval-closure likelihoods (`loglikelihood_PCA` / `_sysrem` / `_high_res`) remain v1 follow-ups.
...
**Test-suite size.** 1,744 tests is healthy but the coverage matrix is finite. Combinatorial coverage over the v0 envelope is real but not exhaustive; the residual risk is configurations that pass POSEIDON parity at the per-case level but combine in ways the sweep does not exercise.
