...
def test_compute_T_Madhu_matches_poseidon():
    ...
    args = dict(
        a1=0.5, a2=0.5, log_P1=-3.0, log_P2=-1.0, log_P3=-0.5, T_set=300.0, P_set=10.0
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        dict(
            ...
            T_set=300.0,
            P_set=1.0e-2,
        ),  # log_P_set ≈ -2, between log_P1=-5 and log_P3=-1
...
def test_add_bulk_component_H_He():
    ...
    He_fraction = 0.17
    X_ours = _atmosphere.add_bulk_component(
        P, T, X_param, 3, 1, 1, ["H", "He"], He_fraction
    )
...
def test_mixing_ratio_categories_matches_poseidon():
    ...
    ours = _atmosphere.mixing_ratio_categories(
        P, X, 1, 1, included, active, CIA, ff, bf
    )
