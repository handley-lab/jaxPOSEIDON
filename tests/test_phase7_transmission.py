...
def _extend_args(**overrides):
    args = dict(
        phi_edge=np.array([-np.pi / 2.0, np.pi / 2.0]),
        theta_edge=np.array([-np.pi / 2.0, np.pi / 2.0]),
        R_s=6.96e8, d=0.0, R_max=7.5e7,
        f_cloud=0.0, phi_0=-90.0, theta_0=90.0,
        N_sectors_back=1, N_zones_back=1, enable_deck=0, N_phi_max=36,
    )
    args.update(overrides)
    return args
...
@pytest.mark.parametrize("overrides", [
    {},                                       # clear 1D
    {"enable_deck": 1, "f_cloud": 1.0,
     "theta_0": -90.0},                        # fully cloudy deck
    {"enable_deck": 1, "f_cloud": 0.4,
     "phi_0": -45.0, "theta_0": -90.0},         # patchy cloud_dim=2
    {"enable_deck": 1, "f_cloud": 0.3,
     "phi_0": 30.0, "theta_0": -30.0},          # cloud splits zone
])
def test_extend_rad_transfer_grids_matches_poseidon(overrides):
    ...
    theirs = p_ext(
        args["phi_edge"], args["theta_edge"], args["R_s"], args["d"],
        args["R_max"], args["f_cloud"], args["phi_0"], args["theta_0"],
        args["N_sectors_back"], args["N_zones_back"], args["enable_deck"],
        args["N_phi_max"],
    )
...
def test_path_distribution_geometric_N_zones1_matches_poseidon():
    ...
    N_phi = 1
    args = dict(
        b=b, r_up=atm["r_up"], r_low=atm["r_low"], dr=atm["dr"],
        i_bot=0, j_sector_back=np.array([0], dtype=np.int64),
        N_layers=N_layers, N_sectors_back=1, N_zones_back=1,
        N_zones=1, N_phi=N_phi,
...
def test_compute_tau_vert_all_branches_match_poseidon():
    ...
    args = dict(
        N_phi=2, N_layers=N_layers, N_zones=2, N_wl=N_wl,
        ...
        kappa_clear=kappa_clear, kappa_cloud=kappa_cloud, dr=dr,
    )
