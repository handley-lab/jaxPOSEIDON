...
@pytest.mark.parametrize(
    "value,grid_start,grid_end,N_grid",
    [
...
def test_closest_index_matches_poseidon(value, grid_start, grid_end, N_grid):
    ...
    assert op.closest_index(value, grid_start, grid_end, N_grid) == p_ci(
        value, grid_start, grid_end, N_grid
    )
...
def test_prior_index_V2_matches_poseidon(value, grid_start, grid_end, N_grid):
    ...
    assert op.prior_index_V2(value, grid_start, grid_end, N_grid) == p_pi2(
        value, grid_start, grid_end, N_grid
    )
...
def test_prior_index_matches_poseidon():
    ...
    grid = np.array([100.0, 150.0, 200.0, 300.0, 500.0, 1000.0, 2000.0, 3500.0])
    for v in (50.0, 100.0, 175.0, 250.0, 999.0, 2500.0, 4000.0):
...
def test_T_interpolation_init_matches_poseidon():
    ...
    T_grid = np.array([100.0, 200.0, 500.0, 1000.0, 2000.0, 3500.0])
    T_fine = np.arange(80.0, 3700.0, 50.0)
    N_T_fine = len(T_fine)
...
def test_wl_initialise_cia_sample_matches_poseidon():
    ...
    ours = op.wl_initialise_cia(
        N_T, N_wl, log_cia, nu_model, nu_cia, len(nu_model), wl_interp="sample"
    )
    theirs = p_init(
        N_T, N_wl, log_cia, nu_model, nu_cia, len(nu_model), wl_interp="sample"
    )
...
def test_wl_initialise_cia_linear_matches_poseidon():
    ...
    log_cia = rng.standard_normal((N_T, len(nu_cia)))
    ours = op.wl_initialise_cia(
        N_T, N_wl, log_cia, nu_model, nu_cia, len(nu_model), wl_interp="linear"
    )
    theirs = p_init(
        N_T, N_wl, log_cia, nu_model, nu_cia, len(nu_model), wl_interp="linear"
    )
...
# ---------------------------------------------------------------------------
def _set_up_sigma_grids(
    N_P_fine=5, N_T=4, N_P=8, N_wl=20, N_nu_opac=100, wl_interp="sample"
):
...
def test_T_interpolate_sigma_matches_poseidon():
    ...
    T_grid = np.array([100.0, 250.0, 500.0, 1000.0, 2000.0])
    T_fine = np.array([80.0, 150.0, 300.0, 700.0, 1500.0, 3500.0])
    y = np.zeros(N_T_fine, dtype=np.int64)
    w_T = op.T_interpolation_init(N_T_fine, T_grid, T_fine, y)
    ours = op.T_interpolate_sigma(
        N_P_fine, N_T_fine, N_T, N_wl, sigma_pre_inp, T_grid, T_fine, y, w_T
    )
    theirs = p_ts(N_P_fine, N_T_fine, N_T, N_wl, sigma_pre_inp, T_grid, T_fine, y, w_T)
...
def test_wl_initialise_cia_linear_at_upper_boundary():
    ...
    out = op.wl_initialise_cia(
        N_T, N_wl, log_cia, nu_model, nu_cia, N_wl, wl_interp="linear"
    )
    ...
    theirs = p_init(N_T, N_wl, log_cia, nu_model, nu_cia, N_wl, wl_interp="linear")
...
def test_T_interpolate_cia_matches_poseidon():
    ...
    T_grid_cia = np.array([200.0, 500.0, 1000.0, 2000.0, 3500.0])
    T_fine = np.array([180.0, 300.0, 700.0, 1500.0, 2500.0, 4000.0])
    y = np.zeros(N_T_fine, dtype=np.int64)
    w_T = op.T_interpolation_init(N_T_fine, T_grid_cia, T_fine, y)
    ours = op.T_interpolate_cia(
        N_T_fine, N_T_cia, N_wl, cia_pre_inp, T_grid_cia, T_fine, y, w_T
    )
    theirs = p_tc(N_T_fine, N_T_cia, N_wl, cia_pre_inp, T_grid_cia, T_fine, y, w_T)
    np.testing.assert_allclose(ours, theirs, atol=0)
