...
@pytest.mark.parametrize("reference_parameter", ["R_p_ref", "P_ref", "R_p_ref+P_ref"])
@pytest.mark.parametrize(
    "offsets_applied", [None, "single_dataset", "two_datasets", "three_datasets"]
)
@pytest.mark.parametrize(
    "error_inflation", [None, "Line15", "Piette20", "Line15+Piette20"]
)
@pytest.mark.parametrize("cloud_case", _CLOUD_CASES)
def test_assign_free_params_full_v0_grid(
    PT_profile, reference_parameter, offsets_applied, error_inflation, cloud_case
):
...
def test_assign_free_params_split_roundtrip_with_canonical_oracle():
    ...
    (_params, _phys, _PT, _X, _cl, _geo, _stel, _hr, _surf, N_params_cum) = (
        assign_free_params(**cfg)
    )
