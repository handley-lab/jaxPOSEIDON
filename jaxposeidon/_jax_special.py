...
- `expn_2(x)`: port of `scipy.special.expn(2, x)` for x >= 0
  ...
  cephes `expn.c`). For x > 50 an asymptotic series for E_2 directly
...
from jax.scipy.special import ndtri  # noqa: F401, E402  (re-exported)
...
def _expn2_asymptotic(x):
    ...
    Valid for large x; the series is divergent, so accumulation stops
    once the absolute term starts growing (optimal truncation for an
    asymptotic series).
    """
    x_safe = jnp.where(x > 0.0, x, 1.0)
    inv = 1.0 / x_safe
...
def expn_2(x):
    ...
    - 1 < x <= 50: same but E_1 by Lentz continued fraction
    - x > 50: asymptotic series for E_2 directly (avoids cancellation)
    """
