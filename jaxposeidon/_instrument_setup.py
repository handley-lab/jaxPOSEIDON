...
Contents:
- `compute_instrument_indices(...)`: ports the reference-data-
  independent slice of POSEIDON `instrument.py:191-318`
  (`init_instrument`). Returns per-bin static metadata
  `(sigma, bin_left, bin_cent, bin_right, norm)` consumed by the
  JAX-pure `_instruments.make_model_data(...)` inside the jit trace.

Phase 0.5.5 will add the photometric dispatch table and full POSEIDON
photometric instrument list to this module.
"""
